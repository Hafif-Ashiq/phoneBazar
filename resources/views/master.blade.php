...
?>
    <nav class="flex w-full justify-around md:gap-[150px] gap-[40px] items-center md:px-[33px] md:pb-[40px] px-[10px] pb-[24px]">
        ...
        <div class="search w-full relative sm:block hidden" >
            <form action="{{url('/search')}}" method="get">
              @csrf
                <input type="text" name="search" id=""  autocomplete="off" placeholder="Search your mobile phone" class="w-full md:text-[24px] text-[16px] text-secondary_text font-poppins lg:px-[25px] px-[15px] py-[12px] rounded-full bg-secondary">
                <input type="submit" value="Search" class=" bg-white absolute right-0 py-[12px] lg:px-[50px] px-[20px] rounded-full md:text-[24px] text-[16px] font-semibold cursor-pointer">
                
            </form>
        </div>
        <div class="flex flex-row items-center justify-end sm:gap-[20px] gap-0">
            <a href="/cart" class="text-white md:text-[24px] sm:text-[20px] text-[18px] font-regular font-poppins hover:underline w-[100px]">Cart ({{$total}})</a>
    ...
    <div class="mb-[20px] px-[40px] hidden"  id="searchForm">
      <form class=" " action="{{url('/search')}}" method="get">
        @csrf
          <input type="text" name="search" id=""  autocomplete="off" placeholder="Search your mobile phone" class="w-full md:text-[24px] text-[14px] text-secondary_text font-poppins lg:px-[25px] px-[15px] py-[10px] rounded-full bg-secondary">
          <input type="submit" value="Search" class=" bg-white absolute right-[40px] py-[10px] px-[20px] rounded-full md:text-[24px] text-[14px] font-semibold cursor-pointer">
      </form>
